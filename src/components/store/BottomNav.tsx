  ...
  const getNavBackground = () => {
    switch (pathname) {
      ...
      case "/store/categories":
        return "from-indigo-400/20 via-blue-400/10 to-cyan-400/20";
      case "/store/cart":
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-10 overflow-hidden">
      ...
      <div className="relative bg-white/70 backdrop-blur-lg border-t border-white/30 shadow-xl">
        <ul className="flex justify-around items-end py-2 px-4 relative">
          {navItems.map(
            ({ href, label, icon: Icon, showBadge, isElevated }, index) => {
              ...
              const getItemColors = () => {
                if (isActive) {
                  switch (href) {
                    ...
                    case "/store/categories":
                      return "text-indigo-600 bg-gradient-to-br from-indigo-100/80 to-blue-100/60 shadow-indigo-200/50";
                    case "/store/cart":
              ...
              return (
                <li key={index} className="flex-1">
                  <Link
                    href={href}
                    className={`
                  flex flex-col items-center relative py-2 px-3 
                  ${isElevated ? "rounded-full" : "rounded-2xl"}
        ...
        {/* Enhanced sliding indicator with gradient - Updated for 5 items */}
        <div
          className={`
          absolute bottom-0 left-0 h-1 rounded-full transition-all duration-700 ease-out
          ${
            pathname === "/store"
              ? "w-1/5 translate-x-0 bg-gradient-to-r from-blue-500 via-blue-600 to-purple-500"
              : pathname === "/store/orders"
              ? "w-1/5 translate-x-full bg-gradient-to-r from-emerald-500 via-emerald-600 to-teal-500"
              : pathname === "/store/categories"
              ? "w-1/5 translate-x-[200%] bg-gradient-to-r from-indigo-500 via-blue-500 to-cyan-500"
              : pathname === "/store/cart"
              ? "w-1/5 translate-x-[300%] bg-gradient-to-r from-orange-500 via-red-500 to-pink-500"
              : pathname === "/store/profile"
              ? "w-1/5 translate-x-[400%] bg-gradient-to-r from-purple-500 via-indigo-500 to-blue-500"
              : "w-0"
          }
